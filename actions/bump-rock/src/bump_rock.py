...
def cmd_fetch(args: argparse.Namespace) -> int:
    ...
    for old_ref, new_ref in resolved_pairs:
        pair = fetch_mod.fetch_pair(old_ref, new_ref, session, timeout=args.timeout)
        paths = fetch_mod.write_pair_to_disk(pair, out_dir)
        written.append(
            {
                "old_url": old_ref.blob_url(),
                "new_url": new_ref.blob_url(),
...
def _resolve_pairs(
    ...
    for old_ref in old_refs:
        ...
        resolved.append((old_res.ref, new_res.ref))
    return resolved
