...
- No code in this repo pushes branches or opens PRs yet — slice B
  (`open-pr`) and slice C (workflow YAML) are pending. Anything that
  touches remote git state must require `--confirm`.
