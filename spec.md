...
1. **Runner size**: building rocks with rockcraft inside `ubuntu-latest` may
   need LXD, which historically needs nested virt or a larger runner. We need
   to confirm `tox -e pack` works on a GH-hosted runner; if not, we either
   use self-hosted runners or a larger GH runner SKU.
